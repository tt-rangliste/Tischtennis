...
function renderBracketHTML(runden, container, type) {
  const MATCH_H  = 72;  // CSS height of each match
  const CSS_GAP  = 8;   // CSS gap between matches in round 0
  const UNIT     = MATCH_H + CSS_GAP; // effective slot per match = 80px

  let html = `<div class="bracket">`;
  runden.forEach((runde, ri) => {
    const label   = roundName(ri, runden.length);
    const topPad  = UNIT * (Math.pow(2, ri) - 1) / 2;
    const gap     = ri === 0 ? CSS_GAP : UNIT * Math.pow(2, ri) - MATCH_H;
