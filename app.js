...
function ergaenzeRunden(runden) {
  // Firebase loescht leere Runden — hier fehlende Folgerunden wieder hinzufuegen
  if (!runden || runden.length === 0) return runden;
  // Anzahl erwarteter Runden = log2(Matches in Runde 0) + 1
  const r0len       = runden[0].length;
  const totalRunden = Math.round(Math.log2(r0len)) + 1;
  let cur           = r0len;
...
function fixTurnier(t) {
  if (!t) return t;
  if (t.hauptrunden) {
    t.hauptrunden = ergaenzeRunden(toArr(t.hauptrunden).map(r => toArr(r)));
  }
  if (t.trostrunden) {
    t.trostrunden = ergaenzeRunden(toArr(t.trostrunden).map(r => toArr(r)));
  }
  if (t.gruppen) {
