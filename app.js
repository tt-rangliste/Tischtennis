...
function ergaenzeRunden(runden) {
  ...
  // cur startet bei der LETZTEN vorhandenen Runde (nicht r0len!)
...
function fixTurnier(t) {
  if (!t) return t;
  if (t.hauptrunden) {
    t.hauptrunden = ergaenzeRunden(toArrSparse(t.hauptrunden).map(r => toArrSparse(r)));
  }
  if (t.trostrunden) {
    t.trostrunden = ergaenzeRunden(toArrSparse(t.trostrunden).map(r => toArrSparse(r)));
  }
  if (t.gruppen) {
